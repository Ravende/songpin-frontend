...
const SideBar = () => {
    ...
    return (
        <SideBarContainer>
            <PinFeatures>
                <StyledButton onClick={() => onItemClick('/home')} noBackground={activePath === '/home'}>
                    <HomeLogo src={sidebar_logo} alt="Home Logo" />
                </StyledButton>
                <StyledButton onClick={() => onItemClick('/home')} isActive={activePath === '/home'}>
                    <Home src={sidebar_home} alt="Home" />
                </StyledButton>
                <StyledButton onClick={() => onItemClick('/search')} isActive={activePath === '/search'}>
...
const StyledButton = styled.button`
    width: 60px;
    height: 60px;
    background: ${({ isActive, noBackground }) => (isActive && !noBackground ? 'var(--light_black, #232323)' : 'none')};
    border: none;
    padding: 0;
    margin: 15px 0;
    cursor: pointer;
